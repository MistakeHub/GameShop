...
<h1>Платформы</h1>
    <b-table striped hover  id="publication-table" :items="publications" :per-page="pagesize" :current-page="currentpage" :fields="fileds">    
       ...
       <b-button variant="danger" size="sm" @click="Remove(row.item.id)" class="mr-1">
        ...
        <b-button  size="sm" bv-modal-example2 @click="ShowEditModal(row.item.id, row.item.titleofplatform)" class="mr-1"> Редактирование</b-button>
          ...
          mounted(){
               ...
               console.log( this.$hostname)
                    ...
                    'set-cookie':document.cookie}}).then(Response=>
                           { this.publications=Response.data.item1;this.countPublication=Response.data.item2; this.isHaveModal=true;this.Type="Platform";   console.log(this.publications)  ;this.fileds=['id', {key:'titleofplatform', label:"Название"}, { key: 'actions', label: 'Actions' }]} ).catch(d=>{
              if(d.response)
